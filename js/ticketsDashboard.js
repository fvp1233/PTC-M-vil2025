...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.filter-btn').forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (filtro === 'todo') {
        ...
        const filtrados = ticketData.filter(ticket => ticket.estado.toLowerCase() === filtro);
  ...
  document.addEventListener('click', e => {
    ...
    if (iconBtn) {
      ...
      const idTexto = tarjeta.querySelector('.id p')?.textContent || '';
      const idNum = idTexto.replace('#', '').trim();
...
function truncateDescriptions(maxChars = 23) {
  ...
  descriptions.forEach(desc => {
    ...
    // Condición para pantallas pequeñas
    if (window.innerWidth <= 768) {
      if (originalText.length > maxChars) {
        let truncated = '';
        const words = originalText.split(' ');
        ...
        // Asegurar que tenga al menos algo visible
        if (truncated.trim().length > 0) {
          desc.textContent = truncated.trim() + '...';
        } else {
          desc.textContent = originalText.slice(0, maxChars) + '...';
        }
      } else {
        desc.textContent = originalText;
      }
    } else {
      // Restaurar texto en pantallas grandes
      desc.textContent = originalText;
    }
  });
}

window.addEventListener('DOMContentLoaded', () => truncateDescriptions());
window.addEventListener('resize', () => truncateDescriptions());
...
document.querySelectorAll('.filter-btn').forEach(button => {
  button.addEventListener('click', () => {
    ...
    if (filtro === 'todo') {
      ...
      const filtrados = ticketData.filter(ticket => 
        ticket.estado.toLowerCase() === filtro
      );
      renderTickets(filtrados);
    }
  });
